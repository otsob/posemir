...
mod tests {
    ...
    fn test_minimal_number_of_mtps() {
        ...
        assert_eq!(mtps[0], Mtp { translator: Point2Di64 { x: 1, y: 0 }, pattern: Pattern::new(&vec![&a, &b, &c]) });
        assert_eq!(mtps[1], Mtp { translator: Point2Di64 { x: 2, y: 0 }, pattern: Pattern::new(&vec![&a, &b]) });
    }
